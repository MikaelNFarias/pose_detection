...
def load_input_data(data_file = 'input_data_avatar.csv'):
    '''
    Import input data (gender and measurments)
    Returns 'gender', abd a list of measurements
    '''
    f = open(os.path.join(INPUT_FILES_DIR, data_file))
    
    measurements = list()
    first_line = f.readline()
    gender = pd.read_csv(os.path.join(INPUT_FILES_DIR,data_file),index_col=0)
