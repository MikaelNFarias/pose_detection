...
def extract_cp(debug: bool = False):
...
def calc_measurements(cp, vertices, facet, vol, label, i):
    ...
    return np.array(measurement_list, dtype='object').reshape(M_NUM, 1)
...
def save_data_csv(meas_names, measurements, label = 'female'):
    ...
    with open(os.path.join(DS_DIR, f'measurements_ANSURI_{label}.csv'), 'w') as outfile:
        # I'm writing a header here just for the sake of readability
